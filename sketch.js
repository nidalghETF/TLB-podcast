// TLB Visualizer - Streaming Version
// ----------------------------------

let song;
let fft;
let isReady = false;

// YOUR AUDIO LINK
const AUDIO_URL = 'https://drive.google.com/uc?export=download&id=1z8DbrpZmgGi3PuCBhTK0cpuawqhKPAZy';

// Visuals Setup
let buildings = [];
const NUM_BUILDINGS = 20;
let nodes = [];
const NUM_NODES = 15;

function setup() {
    // 1. Setup Canvas
    const container = document.getElementById('canvasContainer');
    const canvas = createCanvas(container.offsetWidth, container.offsetHeight);
    canvas.parent('canvasContainer');

    // 2. Setup Audio (STREAMING MODE)
    // We use createAudio instead of loadSound to bypass loading errors
    song = createAudio(AUDIO_URL);
    song.elt.crossOrigin = "anonymous"; // Try to enable CORS for visuals
    
    // When metadata is loaded, we know the duration
    song.elt.onloadedmetadata = () => {
        console.log("Stream Ready!");
        isReady = true;
        
        // Hide loader
        const loader = document.getElementById('loadingScreen');
        if(loader) loader.style.display = 'none';

        // Update UI max time
        const dur = song.duration();
        document.getElementById('totalTime').innerText = formatTime(dur);
        document.getElementById('progressBar').max = dur;
    };

    // 3. Setup FFT (Visualizer)
    fft = new p5.FFT(0.8, 64);
    // Important: Connect the streaming audio to the analyzer
    // Note: If Drive blocks CORS, visuals might be flat, but audio will play.
    fft.setInput(song);

    // 4. Create Visual Objects
    let bWidth = width / NUM_BUILDINGS;
    for (let i = 0; i < NUM_BUILDINGS; i++) {
        buildings.push({
            x: i * bWidth,
            w: bWidth - 2,
            h: random(50, 200)
        });
    }

    for (let i = 0; i < NUM_NODES; i++) {
        nodes.push({
            x: random(width),
            y: random(height / 2),
            vx: random(-0.5, 0.5),
            vy: random(-0.5, 0.5),
            size: random(5, 15)
        });
    }

    // 5. Connect Buttons
    setupUI();
}

function draw() {
    background(10, 14, 23); // Dark Navy

    if (!isReady) return;

    // Analyze Audio
    let spectrum = fft.analyze();
    let bassEnergy = fft.getEnergy("bass");
    let trebleEnergy = fft.getEnergy("treble");

    // DRAW NODES (Top Half)
    stroke(0, 255, 157, 50);
    for (let i = 0; i < nodes.length; i++) {
        let n = nodes[i];
        n.x += n.vx; n.y += n.vy;
        if (n.x < 0 || n.x > width) n.vx *= -1;
        if (n.y < 0 || n.y > height/2) n.vy *= -1;

        // Connections
        for (let j = i + 1; j < nodes.length; j++) {
            let other = nodes[j];
            if (dist(n.x, n.y, other.x, other.y) < 150) {
                line(n.x, n.y, other.x, other.y);
            }
        }
        // Node dots
        noStroke();
        fill(0, 255, 157, map(trebleEnergy, 0, 255, 100, 255));
        circle(n.x, n.y, n.size + map(trebleEnergy, 0, 255, 0, 5));
    }

    // DRAW CITY (Bottom Half)
    noStroke();
    for (let i = 0; i < buildings.length; i++) {
        let b = buildings[i];
        // Map building index to frequency spectrum
        let specIndex = floor(map(i, 0, NUM_BUILDINGS, 0, 16)); 
        let amp = spectrum[specIndex] || 0;
        
        let currentH = b.h + map(amp, 0, 255, 0, height/3);

        // Building Bar
        fill(0, 243, 255, 150);
        rect(b.x, height - currentH, b.w, currentH);
        
        // Window Accent
        fill(255, 255, 255, 50);
        rect(b.x + 5, height - currentH + 10, b.w - 10, 5);
    }

    // Update UI Progress Bar if playing
    if (!song.elt.paused) {
        updateProgressBar();
    }
}

function windowResized() {
    const c = document.getElementById('canvasContainer');
    resizeCanvas(c.offsetWidth, c.offsetHeight);
}

// --- UI LOGIC ---
function setupUI() {
    const playBtn = document.getElementById('playBtn');
    const restartBtn = document.getElementById('restartBtn');
    const slider = document.getElementById('progressBar');
    const volSlider = document.getElementById('volSlider');

    playBtn.addEventListener('click', () => {
        if (!isReady) return;
        // NOTE: Browsers block audio until user interaction.
        // Clicking this button counts as the interaction.
        if (song.elt.paused) {
            song.play(); // Start stream
            song.volume(parseFloat(volSlider.value)); // Ensure volume
            playBtn.innerText = "PAUSE";
            playBtn.classList.add("main-play-active");
        } else {
            song.pause();
            playBtn.innerText = "PLAY";
            playBtn.classList.remove("main-play-active");
        }
    });

    restartBtn.addEventListener('click', () => {
        if (!isReady) return;
        song.time(0); // Jump to start
        if (song.elt.paused) {
            song.play();
            playBtn.innerText = "PAUSE";
        }
    });

    slider.addEventListener('input', () => {
        if (!isReady) return;
        song.time(parseFloat(slider.value));
    });

    volSlider.addEventListener('input', () => {
        if (song) song.volume(parseFloat(volSlider.value));
    });
}

function updateProgressBar() {
    const slider = document.getElementById('progressBar');
    const timeDisplay = document.getElementById('currentTime');
    let curr = song.time();
    slider.value = curr;
    timeDisplay.innerText = formatTime(curr);
}

function formatTime(seconds) {
    let m = floor(seconds / 60);
    let s = floor(seconds % 60);
    return m + ":" + (s < 10 ? "0" : "") + s;
}
